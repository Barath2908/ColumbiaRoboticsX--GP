...
% and animates the results, including an external periodic disturbance.
...
A = 10;                  % Amplitude of disturbance torque (N·m)
f = 0.5;                 % Frequency of disturbance (Hz)
...
params.T_F = T_F;
params.T_disturbance = T_disturbance;
params.dt = dt;
params.t_start = t_start;
params.t_end = t_end;

% Define the ODE function handle
ode_func = @(t, y) gait_initiation_dynamics_ext_disturbance(t, y, params);
...
T_PID = Kp * error + Ki * integral + Kd * derivative_of_error;  % Updated derivative term
T_F_values = arrayfun(T_F, t);                           % T_F(t) over time
T_disturbance_values = arrayfun(T_disturbance, t);       % Disturbance torque over time

% Calculate Total Ankle Torque
T_total = T_F_values + T_PID;

% Calculate Net Torque (including disturbance)
Net_Torque = T_total + T_disturbance_values;
...
plot(t, T_disturbance_values, 'c-', 'LineWidth', 2);   % Plot disturbance torque
plot(t, Net_Torque, 'k-', 'LineWidth', 2);             % Plot net torque
xlabel('Time (s)');
ylabel('Torque (N·m)');
legend('T_{PID}', 'T_F', 'T_{disturbance}', 'Net Torque');
...
animation_speed = 6.25;    % 1 for real-time, >1 for faster, <1 for slower
